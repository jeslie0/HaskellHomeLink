...
epollCreate size = do
  epfd <- liftIO $ c_epoll_create (fromIntegral size)
  if epfd < 0
    then
      ExceptT . pure . Left $
...
epollCreate1_ = (either throwIO return <=< runExceptT) . epollCreate1
...
epollCtl_ epoll epollOp fd =
  (either throwIO return <=< runExceptT) . epollCtl epoll epollOp fd
...
epollWait epoll maxEvents timeout = do
  (Fd epfd) <- liftIO $ epollToFd epoll
  ExceptT $ allocaArray maxEvents $ \eventsPtr -> do
    n <-
...
epollWait_ epoll maxEvents = do
  (either throwIO return <=< runExceptT) . epollWait epoll maxEvents
